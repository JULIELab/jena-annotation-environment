 ...
 *
 * Author: tomanek (based on code by Menno Rubin)
 * 
 * Current version: 2.0
 * Since version:   0.7
 *
 ...
 * 
 * Code from Menno used to create basedata- and style-files from the AL output
 *   
 **/
...
/**
 * 
 * (list.sel) Enhanced to typographically change non-markable parts of abstract
 * (--jw)
 */
...
// - Directory pathname containing tokenized texts. (E.g.
...
// - alsession.txt
//
// The class contains only the following two public methods:
// main() -- For calling as stand-along program
// createFiles() -- For calling from Java program.
import java.io.*;
import java.util.*;
...
public class MkALStyles {

	private static Logger logger = AnnoEnvLogger
			.getLogger("de.julielab.annoenv.conversionsMkALStyles");
			...
			boolean _wTID) // IN (show textID for each text in MMAX display)
	{
		...
		while (iS.hasNext()) {
			...
			if (se.bAnn) {
				...
				out.println("<xsl:value-of select=\"mmax:startSubscript()\"/>");
				out.println("<xsl:value-of select=\"mmax:startColor('" + Constants.MMAX_STYLE_CONTEXT_COLOR + "')\"/>");
				...
				out.println("");
				out.println("</xsl:text>");
				out.println("<xsl:value-of select=\"mmax:endColor('" + Constants.MMAX_STYLE_CONTEXT_COLOR + "')\"/>");
				out.println("<xsl:value-of select=\"mmax:endSubscript()\"/>");
	...
	// pwSentlvl OUT PrintWriter object to write sentence_level.xml (markable)
	// to
	...
	public static String createFiles(File strPathTok, File sellistFile,
			String levelname, PrintWriter pwStyle, PrintWriter pwBasedata,
			PrintWriter pwSentlvl, PrintWriter pwAlsess, int PID)
			throws IOException {

		String erm = null;

		// ,-------------------------
		// | Get input TID:SID list
		// `-------------------------

		LinkedList<TIDSID> lIn = new LinkedList<TIDSID>();

		erm = readListSel(lIn, sellistFile.toString());
		if (erm != null) {
			logger.severe(PID + "#Fail read '" + sellistFile.toString() + "': "
					+ erm);
			throw new IOException("Fail read '" + sellistFile.toString()
					+ "': " + erm);
		}

		logger.fine(PID + "#nIn = " + lIn.size());
		...
		// logger.fine("lFile: n = " + lFile.size());
		// Iterator<String> iFs = lFile.iterator();
		// while (iFs.hasNext()) {
		// String filename = iFs.next();
		...
		LinkedList<SE> lS = new LinkedList<SE>();
		{
			...
			while (iF.hasNext()) {
				...
				String pathname = strPathTok.toString() + File.separator
						+ filename;
				erm = lse_readFile(lS, pathname, filename);
				if (erm != null) {
					logger
							.severe(PID + "#Fail read '" + pathname + "': "
									+ erm);
					throw new IOException("Fail read '" + pathname + "': "
							+ erm);
				}
			}
		}
		...
		// Mark in list<SE> the sentences that are to be annotated
		{
			...
			while (iIn.hasNext()) {
				...
				if (!ok) {
					logger.severe(PID + "#Internal error lse_setAnn("
							+ entry.textID + "," + entry.sentID + ")");
					throw new IllegalStateException(
							"Internal error lse_setAnn(" + entry.textID + ","
									+ entry.sentID + ")");
				}
			}
		}
		...
		logger.fine(PID + "#nWordsTot = " + nWords);
	...
	/*
	 * public static void main(String[] args) { String erm = null; //
	 * ,------------------------- // | Cmd line args //
	 * `-------------------------
	 * 
	 * final int N_REQ_ARGS = 4;
	 * 
	 * char cWhat = 'S'; // Styles // 'B' = basedata // 'M' = sentence_level.xml //
	 * 'D' = alsession.txt
	 * 
	 * boolean _v = false; boolean _t = false;
	 * 
	 * if (args.length < N_REQ_ARGS) { usage(); System.exit(-1); }
	 * 
	 * String strPathTok = args[0]; int lenPT = strPathTok.length(); assert
	 * ((lenPT > 0) && (strPathTok.charAt(lenPT - 1) != '/'));
	 * 
	 * String listselpath = args[1]; // Annotation level name to mention in
	 * generated Styles/*.xsl file // String levelname = "protein"; // String
	 * levelname = "LEVELNAME"; // String levelname = "NamedEntity"; String
	 * levelname = args[2]; assert (levelname.length() > 0);
	 * 
	 * assert (args[3].length() > 0); cWhat = args[3].charAt(0);
	 * 
	 * int iOpt; for (iOpt = N_REQ_ARGS; iOpt < args.length; iOpt++) { String
	 * sOpt = args[iOpt]; if (sOpt.length() < 2) { usage(); System.exit(-1); }
	 * if (sOpt.charAt(0) != '-') { usage(); System.exit(-1); } switch
	 * (sOpt.charAt(1)) { case 'v': _v = true; break; case 't': _t = true;
	 * break; default: usage(); System.exit(-1); break; } }
	 */
	/*
	 ...
	 * "/data/data/data_corpora/genomics/GeneOnto/abstracts/tok" lIn.add( new
	 * TIDSID( "1618911", 3 ) ); lIn.add( new TIDSID( "12384566", 3 ) ); //Test
	 * '>' in basedata lIn.add( new TIDSID( "7707319", 4 ) ); //Two items with
	 * same TID lIn.add( new TIDSID( "7707319", 2 ) ); lIn.add( new TIDSID(
	 * "11516400", 0 ) ); //1st sentence in file lIn.add( new TIDSID( "8676079",
	 * 1 ) ); lIn.add( new TIDSID( "1400430", 1 ) ); lIn.add( new TIDSID(
	 * "1400417", 8 ) ); lIn.add( new TIDSID( "16188968", 2 ) ); lIn.add( new
	 * TIDSID( "1391960", 1 ) ); lIn.add( new TIDSID( "12381793", 0 ) );
	 * lIn.add( new TIDSID( "11494046", 1 ) ); lIn.add( new TIDSID( "7687629",
	 * 12 ) ); //Last sentence in file lIn.add( new TIDSID( "11497433", 1 ) );
	 * lIn.add( new TIDSID( "8659123", 2 ) ); lIn.add( new TIDSID( "16174773", 1 ) );
	 * lIn.add( new TIDSID( "9486644", 0 ) ); lIn.add( new TIDSID( "10837363", 1 ) );
	 * //2*sameTID, adjacent s.s lIn.add( new TIDSID( "10837363", 2 ) );
	 * lIn.add( new TIDSID( "10847589", 1 ) ); } else { erm = readListSel( lIn,
	 * listselpath ); if ( erm != null ) { ERR( "Fail read '" + listselpath +
	 * "': " + erm ); System.exit(-1); } }
	 * 
	 ...
	 * lFile = new HashSet<String>(); { Iterator<TIDSID> iIn = lIn.iterator();
	 * while ( iIn.hasNext() ) { TIDSID entry = iIn.next(); lFile.add(
	 * entry.textID ); } }
	 * 
	 * //Dump the set lFile if(_v) { OUT( "lFile: n = " + lFile.size() );
	 * Iterator<String> iF = lFile.iterator(); while ( iF.hasNext() ) { String
	 * filename = iF.next(); OUT( "file: " + filename ); } }
	 * 
	 * 
	 * 
	 ...
	 * strPathTok + File.separator + filename; erm = lse_readFile( lS, pathname,
	 * filename ); if ( erm != null ) { ERR( "Fail read '" + pathname + "': " +
	 * erm ); System.exit(-1); } } }
	 * 
	 * 
	 * //Mark in list<SE> the sentences that are to be annotated { Iterator<TIDSID>
	 * iIn = lIn.iterator(); while ( iIn.hasNext() ) { TIDSID entry =
	 * iIn.next(); boolean ok = lse_setAnn( lS, entry.textID, entry.sentID ); if (
	 * !ok ) { ERR( "Internal error lse_setAnn(" + entry.textID + "," +
	 ...
	 * "===========================================================" ); OUT(
	 * "===========================================================" ); } //
	 * WRITE OUTPUT TO STDOUT FOR TESTING // In public func: Write to outBD,
	 * outSL, outStyle if != null.
	 * 
	 ...
	 * sentence_level.xml contents to stdout { lse_printSentencelevel( lS, pwOut ); }
	 * if ( cWhat == 'S' ) //Write Styles/*.xsl contents to stdout {
	 * lse_printStyle( lS, levelname, pwOut, true ); } if ( cWhat == 'D' )
	 * //Write own 'alsession.txt' contents to stdout {
	 ...
	 */
	/*
	 * // Write one of the output files to stdout // ^^^ ^^^^^^ PrintWriter
	 * pwOut = new PrintWriter(System.out);
	 * 
	 * PrintWriter pwStyle = null; PrintWriter pwBasedata = null; PrintWriter
	 * pwSentlvl = null; PrintWriter pwAlsess = null;
	 * 
	 * switch (cWhat) { case 'B': pwBasedata = pwOut; break; case 'M': pwSentlvl =
	 * pwOut; break; case 'S': pwStyle = pwOut; break; case 'D': pwAlsess =
	 * pwOut; break; }
	 * 
	 ...
	 * pwOut.close(); // <-- !!
	 * 
	 * if (erm != null) { ERR(erm); System.exit(-1); }
	 * 
	 * System.exit(0); }
	 */
}
